...
typedef struct {
    ...
    int alert_level;       /* final G26 severity: 1 normal, 2 attention, 3 alarm */
    int line_branch;
    int patient_branch;
...
void on_message(struct mosquitto *mosq, void *userdata, const struct mosquitto_message *msg)
{
    ...
    ts.remaining_ml = -1;
    ts.remaining_min = -1;
    if (!get_int_from_json(payload, "final_alert_level", &ts.alert_level)) {
        int physical_level = 0;
        if (get_int_from_json(payload, "alert_level", &physical_level)) {
            ts.alert_level = physical_level + 1;
            if (ts.alert_level > 3) ts.alert_level = 3;
        } else {
            ts.alert_level = alarm ? 2 : 1;
        }
    }
    ...
    {
        static const char *level_name[4] = { "UNKNOWN", "NORMAL",
                                             "ATTENTION", "ALARM" };
