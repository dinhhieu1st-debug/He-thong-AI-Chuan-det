...
CREATE TABLE IF NOT EXISTS beds (
  ...
  final_alert_level TINYINT UNSIGNED NULL,
  line_branch BOOLEAN NOT NULL DEFAULT FALSE,
  patient_branch BOOLEAN NOT NULL DEFAULT FALSE,
  spo2_alarm BOOLEAN NOT NULL DEFAULT FALSE,
  heart_rate_alarm BOOLEAN NOT NULL DEFAULT FALSE,
  drop_training_samples TINYINT UNSIGNED NOT NULL DEFAULT 0,
  vitals_training_samples TINYINT UNSIGNED NOT NULL DEFAULT 0,
  alerts_armed BOOLEAN NOT NULL DEFAULT TRUE,
...
-- Time-series vitals history. Every numeric channel the device reports gets
-- its own column so the bed detail view can chart each one independently
-- (see Api/BedEndpoints.cs `/history` and wwwroot/js/charts.js).
--
-- NULL means "no trustworthy reading at this instant" - the signal was lost
-- or the channel is not installed. That is deliberately NOT stored as 0:
-- a chart must show a GAP there, because drawing 0 bpm would look like a
-- flatlining patient rather than a detached sensor.
CREATE TABLE IF NOT EXISTS vital_samples (
  ...
  drops_per_min INT NULL,                -- raw measured drop rate
  target_flow_ml_h INT NULL,             -- doctor's target AT THAT MOMENT, so an
  target_drops_per_min INT NULL,         --   old ratio stays interpretable later
  line_blocked TINYINT(1) NOT NULL DEFAULT 0,
  ae_alarm TINYINT(1) NOT NULL DEFAULT 0,
