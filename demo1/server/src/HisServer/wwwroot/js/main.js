...
(async function main() {
  ...
  function switchTab(tab) {
    ...
    sessionStorage.setItem("his.activeTab", tab);
  ...
  const savedTab = sessionStorage.getItem("his.activeTab");
  const savedButton = savedTab
    ? document.querySelector(`.nav-btn[data-tab="${savedTab}"]`)
    : null;
  if (savedButton) switchTab(savedTab);
  else if (landing) switchTab(landing[1]);
