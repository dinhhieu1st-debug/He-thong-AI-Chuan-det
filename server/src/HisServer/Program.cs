...
// Cloudflare Tunnel terminates HTTPS and forwards to this process over local
// HTTP. Trust the standard forwarded headers from the local proxy so redirects,
// authentication cookies and generated URLs retain the public HTTPS scheme.
// ASP.NET Core's default KnownProxies list trusts loopback only, which matches a
// cloudflared process running on this machine without trusting arbitrary peers.
...
builder.Services.Configure<HttpIngestionOptions>(builder.Configuration.GetSection("HttpIngestion"));
...
builder.Services.AddSingleton<BedTcpIngestionService>();
builder.Services.AddHostedService(serviceProvider =>
    serviceProvider.GetRequiredService<BedTcpIngestionService>());
...
builder.Services
    ...
    .AddCookie(options =>
    {
        ...
        // Secure through Cloudflare (X-Forwarded-Proto: https), while keeping
        // direct local HTTP login usable for diagnostics and development.
