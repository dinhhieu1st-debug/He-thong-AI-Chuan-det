...
static void publish_command(const char *line)
{
    ...
    if (strcmp(command, "rescan_devices") == 0) {
        ...
        } else if (strcmp(command, "set_drop_level") == 0 && json_integer(line, "value", &value)) {
            if (value < 1 || value > 3) return;
            snprintf(payload, sizeof(payload), "{\"server_drop_level\":%ld}", value);
        } else if (strcmp(command, "set_vitals_test_mode") == 0 && json_integer(line, "value", &value)) {
            if (value != 0 && value != 2 && value != 3) return;
            snprintf(payload, sizeof(payload), "{\"vitals_test_mode\":%ld}", value);
