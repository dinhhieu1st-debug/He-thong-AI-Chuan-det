...
# Use the SLT tooling to find the install paths. On Windows, VS Code may be
# launched without the SLT installation directory on PATH, so also probe the
# standard Silicon Labs installation locations.
find_program(SLT_EXECUTABLE
             NAMES slt.exe slt
             HINTS
               "$ENV{LOCALAPPDATA}/Programs/SiliconLabsTool"
               "$ENV{LOCALAPPDATA}/Programs/SimplicityInstaller/resources")

if (NOT SLT_EXECUTABLE)
  message(FATAL_ERROR "Silicon Labs Tool (slt) was not found. Install SLT or add it to PATH.")
endif ()

execute_process(COMMAND "${SLT_EXECUTABLE}" where gcc-arm-none-eabi
                OUTPUT_VARIABLE "ARM_GCC_SLT_PATH"
                OUTPUT_STRIP_TRAILING_WHITESPACE)

execute_process(COMMAND "${SLT_EXECUTABLE}" where commander
                OUTPUT_VARIABLE "COMMANDER_SLT_PATH"
                OUTPUT_STRIP_TRAILING_WHITESPACE)

execute_process(COMMAND "${SLT_EXECUTABLE}" where ninja
...
elseif (WIN32)
  message(FATAL_ERROR "GNU Arm Embedded toolchain was not found by SLT.")
elseif (APPLE)
