...
static void updateCurrentImageAttributes(sl_zigbee_af_ota_image_id_t* imageId)
{
  ...
  && !defined(ZCL_USING_OTA_BOOTLOAD_CLUSTER_IMAGE_TYPE_ID_ATTRIBUTE)  \
  && !defined(ZCL_USING_OTA_BOOTLOAD_CLUSTER_CURRENT_FILE_VERSION_ATTRIBUTE)
  UNUSED_VAR(imageId);
#endif
#if defined(ZCL_USING_OTA_BOOTLOAD_CLUSTER_MANUFACTURER_ID_ATTRIBUTE)
                                          ...
                                          sl_zigbee_af_cluster_command_t* message)
{
  ...
  switch (commandId) {
    ...
    case ZCL_QUERY_NEXT_IMAGE_REQUEST_COMMAND_ID: {
      // [SmartIV-DEBUG] state snapshot at the moment the response arrives.
      // Diagnostics only - no behavioural change. Remove to restore stock SDK.
      // NOTE: waitingForResponse is deliberately not printed here; it was already
      // cleared unconditionally at the top of this function (see line ~1044).
      otaPrintln("OTA DBG: QNI rx, state=%s, defaultResp=%d, errors=%d",
                 bootloadStateNames[currentBootloadState],
                 (defaultResponse ? 1 : 0),
                 errors);
      otaPrintln("OTA DBG:   src=0x%04X srv=0x%04X myEp=%d srvEp=%d",
                 message->source,
                 serverNodeId,
                 myEndpoint,
                 serverEndpoint);
                           ...
                           bool queryErrorOccurred)
{
  ...
  if (sendQuery == SEND_QUERY_NOW) {
    ...
    // [SmartIV-DEBUG] setTimer() will override the query delay with
    // MESSAGE_TIMEOUT_MS because sendMessage() sets waitingForResponse first.
    // Printing it resolves the effective response window at runtime.
    otaPrintln("OTA DBG: sending QNI request, response window=%d ms, errors=%d",
...
static void updateImageTypeIdAttribute(uint16_t imageTypeId)
{
  ...
  (void) sl_zigbee_af_write_attribute(myEndpoint,
                                      ...
                                      (uint8_t*)&imageTypeId,
                                      ZCL_INT16U_ATTRIBUTE_TYPE);
#else // !defined(ZCL_USING_OTA_BOOTLOAD_CLUSTER_IMAGE_TYPE_ID_ATTRIBUTE)
  UNUSED_VAR(imageTypeId);
